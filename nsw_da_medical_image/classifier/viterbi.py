...
def predicted_phase_plot(predicted_classes,predicted_classes_viterbi,labels,phase_list,viz_fold,video,fontsize=30):
    all_raw_seq = [predicted_classes,predicted_classes_viterbi,labels]
    all_subplot_names = ["Raw","with Viterbi \n post-processing","Ground-truth"]
    ...
    for i in range(len(phase_list)):
        leg_handles += [axs[0].add_patch(Rectangle((0,0),0.001,0.001,label=str(phase_list[i]).replace("Phase.",""),color=color_map(i/len(phase_list))))]
    ...
    for i,(raw_pred_seq,subplot_name) in enumerate(zip(all_raw_seq,all_subplot_names)):
        ...
        axs[i].set_ylabel(subplot_name,fontsize=fontsize,rotation=0)
        xticks = list(np.arange(len(raw_pred_seq)//100)*100)+[len(raw_pred_seq)]
        axs[i].set_xticks(xticks,xticks,fontsize=fontsize)
        #axs[i].set_yticks([],[])
        axs[i].set_yticklabels(labels=[],fontdict=[])
        axs[i].yaxis.set_label_coords(-0.23, 0.4)

    fig.legend(bbox_to_anchor=(0.95, 0.95),prop={'size': int(fontsize*0.75)})
    #fig.legend()
    fig.tight_layout()
    plt.subplots_adjust(hspace=0.3,right=0.775,bottom=0.145,left=0.25)
